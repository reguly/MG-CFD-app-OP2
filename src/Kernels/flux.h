    ...
    float *fluxes_b)
{
    ...
    float factor_x = 0.5f * edge_weight[0],
           factor_y = 0.5f * edge_weight[1],
           factor_z = 0.5f * edge_weight[2];
...
inline void compute_flux_edge_kernel(
    ...
    float *fluxes_b)
{
  float ewt = sqrt(edge_weight[0]*edge_weight[0] +
  ...
  float speed_a = sqrt(speed_sqd_a);
