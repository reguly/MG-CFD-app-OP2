...
int main(int argc, char** argv)
{
    ...
    char* op_name = alloc<char>(100);
    {
        ...
        for (int i=0; i<levels; i++) {
            ...
            p_variables[i] = op_decl_dat_temp_char(op_nodes[i], NVAR, "float", sizeof(double), op_name);
            sprintf(op_name, "p_old_variables_L%d", i);
            p_old_variables[i] = op_decl_dat_temp_char(op_nodes[i], NVAR, "float", sizeof(double), op_name);
            sprintf(op_name, "p_residuals_L%d", i);
            p_residuals[i] = op_decl_dat_temp_char(op_nodes[i], NVAR, "float", sizeof(double), op_name);

            if (!conf.legacy_mode) {
                // Need to calculate cell volumes:
                sprintf(op_name, "p_volumes_L%d", i);
                p_volumes[i] = op_decl_dat_temp_char(op_nodes[i], 1, "float", sizeof(double), op_name);
            }

            sprintf(op_name, "p_step_factors_L%d", i);
            p_step_factors[i] = op_decl_dat_temp_char(op_nodes[i], 1, "float", sizeof(double), op_name);

            sprintf(op_name, "p_fluxes_L%d", i);
            p_fluxes[i] = op_decl_dat_temp_char(op_nodes[i], NVAR, "float", sizeof(double), op_name);
    ...
    for (int i=0; i<levels; i++) {
        ...
        if (!conf.legacy_mode) {
            ...
            op_par_loop_calculate_cell_volumes("calculate_cell_volumes",op_edges[i],
                        ...
                        op_arg_dat(p_edge_weights[i],-1,OP_ID,3,"float",OP_INC),
    ...
    while(i < conf.num_cycles)
    {
        ...
        for (rkCycle=0; rkCycle<RK; rkCycle++)
        {
            ...
            op_par_loop_zero_5d_array_kernel("zero_5d_array_kernel",op_nodes[level],
                        op_arg_dat(p_fluxes[level],-1,OP_ID,5,"float",OP_WRITE));
        }
    ...
    if (conf.validate_result) {
        ...
        if (!value_check_failed) {
            ...
            for (int l=0; l<levels; l++) {
                ...
                op_dat variables_difference = op_decl_dat_temp_char(op_nodes[l], NVAR, "float", sizeof(double), op_name);
